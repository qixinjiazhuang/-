      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            ...
            <form action="/admin/mechanic/appointment" method="get">
                <div class="row"> 
                    <!-- select -->
                    <div class="col-md-2">
                        <div class="form-group">
                            <a href="/admin/mechanic/addapp">添加预约</a>
                        </div>
                    </div>
              ...
              <table id="example2" class="table table-bordered table-hover" style="table-layout:fixed;">
                ...
                <tr>
                  ...
                  <th style="text-align:center;">技工</th>
                ...
                <?php if(is_array($res)): foreach($res as $key=>$v): if($v['delete']==1){ ?>
                    ...
                    <td style="text-overflow:ellipsis;text-align:center;"><?=date('Y-m-d H:i:s',$v['time'])?></td>
	                  <td style="text-overflow:ellipsis;text-align:center;">
	                  	  <a href="/admin/mechanic/editapp?id=<?php echo ($v["id"]); ?>">编辑</a> | 
	                      <a onClick="javascript:del()" href="javascript:del()">删除</a> | 
                        <?php if($v['a_status'] == 1){ ?>
                          <a href="/admin/mechanic/chuli?id=<?php echo ($v["id"]); ?>">处理预约</a> 
                        <?php }else{ ?>
                          <a href="/admin/mechanic/quxiao?id=<?php echo ($v["id"]); ?>">取消预约</a>
                        <?php } ?>
                        <SCRIPT LANGUAGE=javascript>
                            function del() {
                            var msg = "您真的确定要删除？";
                                if (confirm(msg)==true){
                                        window.location.href="/admin/mechanic/deleteapp?id=<?php echo ($v["id"]); ?>"
                                    }else{
                                        window.location.href="/admin/mechanic/index"
                                }
                            }
                        </SCRIPT> 
	                  </td>
